define(function (require) {
  var module = require('ui/modules').get('kibana/kbn_relation_vis', ['kibana']);
  var d3 = require('d3');
  ...
  module.controller('KbnRelationVisController', function ($scope, $element, $rootScope, Private) {
    $scope.$watch('vis.params.cypher', function (cypher) {
      if (!cypher) return;

      $.ajax({
        type: "POST",
        url: "http://192.168.0.108:7474/db/data/cypher",
        contentType: "application/json",
        data: JSON.stringify({
          "query" : cypher,
          "params": {}
          //"query":"MATCH (czrk:`czrk`)-[r]-(czrk1) WHERE czrk.SFZH=\"422128197709100054\" RETURN czrk,r,czrk1 limit 900"
        }),
 ...
 /* var svgRoot = $element[0];
  ...
  var _buildVis = function (root) {
    ...
    $scope.$watch('esResponse', function (resp) {
      	if (resp) {
        	var chartData = relationAggResponse($scope.vis, resp);
        	_render(chartData);
      	}
    });
